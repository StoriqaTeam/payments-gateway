...
pub trait TransactionsClient: Send + Sync + 'static {
    ...
    fn refresh_rate(&self, input: RefreshRate) -> Box<Future<Item = RateRefresh, Error = Error> + Send>;
...
impl TransactionsClient for TransactionsClientImpl {
    ...
    fn refresh_rate(&self, input: RefreshRate) -> Box<Future<Item = RateRefresh, Error = Error> + Send> {
        let client = self.clone();
        let url = format!("/rate/refresh");
        Box::new(
            serde_json::to_string(&input)
                .map_err(ectx!(ErrorSource::Json, ErrorKind::Internal => input))
                .into_future()
                .and_then(move |body| client.exec_query::<RateRefresh>(&url, Some(body), Method::POST)),
        )
    }
    fn get_fees(&self, input: GetFees) -> Box<Future<Item = Fees, Error = Error> + Send> {
        let client = self.clone();
...
impl TransactionsClient for TransactionsClientMock {
    ...
    fn refresh_rate(&self, _input: RefreshRate) -> Box<Future<Item = RateRefresh, Error = Error> + Send> {
        Box::new(Ok(RateRefresh::default()).into_future())
    }
