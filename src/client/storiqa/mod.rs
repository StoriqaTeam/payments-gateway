    ...
    fn exec_query<T: for<'de> Deserialize<'de> + Send>(&self, query: &str) -> impl Future<Item = T, Error = Error> + Send {
        ...
        Request::builder()
            ...
            .and_then(|bytes| {
                ...
                String::from_utf8(bytes).map_err(ewrap!(ErrorContext::ResponseUTF8, ErrorKind::Internal, bytes_clone))
            })
            .and_then(|string| serde_json::from_str::<T>(&string).map_err(ewrap!(ErrorContext::ResponseJson, ErrorKind::Internal, string)))
    }
}
